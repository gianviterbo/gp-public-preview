<?php
/**
 * Plugin Name:       GP Public Preview
 * Plugin URI:        https://github.com/gianviterbo/gp-public-preview
 * Description:       Fork of DS Public Post Preview with per-link lifetimes (1–168h), one-click link refresh/regeneration and an enforced server-side expiry gate. Create shareable preview links for drafts.
 * Version:           1.0.0
 * Requires at least: 6.0
 * Requires PHP:      7.2
 * Author:            Gadget Pilipinas
 * Author URI:        https://www.gadgetpilipinas.net/
 * License:           GPL-2.0-or-later
 * License URI:       https://www.gnu.org/licenses/gpl-2.0.html
 * Text Domain:       public-post-preview
 * Domain Path:       /languages
 *
 * This is a fork of the GPLv2+ plugin "Public Post Preview" by Dominik Schilling
 * (https://wordpress.org/plugins/public-post-preview/). Fork additions:
 *   - per-post link lifetime in hours (default from Settings → Reading),
 *   - regenerate/refresh action that immediately expires old shared copies,
 *   - hard front-end expiry gate (403 once the lifetime is up).
 * Option keys (public_post_preview, public_post_preview_expiration_time) and
 * the text domain are kept from upstream so state and translations survive.
 */
...
class GP_Public_Preview {
	...
	public static function init() {
		...
		// GP fork: expose lifetime meta through the REST API (per post type).
		foreach ( self::get_post_types() as $gp_ppp_pt ) {
			register_post_meta(
				$gp_ppp_pt,
				'gp_ppp_hours',
				array(
		...
		if ( ! is_admin() ) {
			...
			add_action( 'wp_ajax_gp_ppp_save', array( __CLASS__, 'ajax_gp_save' ) );
	...
	static function register_settings_ui() {
		...
		add_settings_field(
			'public_post_preview_expiration_time',
			__( 'Default preview link lifetime (hours)', 'public-post-preview' ),
			static function() {
				...
				<p class="description"><?php _e( 'Default hours a shared preview link stays valid (1-168). Each post can override this in the editor, and links can be regenerated to expire old copies instantly. Enforced server-side.', 'public-post-preview' ); ?></p>
				<?php
			},
			'reading',
			'public_post_preview',
	...
	public static function enqueue_script( $hook_suffix ) {
		...
		if ( get_current_screen()->is_block_editor() ) {
			...
			// GP fork: granular lifetime panel for the block editor.
			wp_enqueue_script(
				'gp-public-preview-editor',
				plugins_url( 'js/gp-editor.js', __FILE__ ),
			...
			wp_localize_script(
				...
				array(
					...
					'postId'     => $post->ID,
					'nonce'      => wp_create_nonce( 'public-post-preview_' . $post->ID ),
					'hours'      => self::get_link_hours( $post->ID ),
	...
	private static function get_checkbox_html( $post ) {
		...
		<?php if ( $enabled ) : ?>
			<span class="description"><?php printf( esc_html__( 'This link expires around %s (link lifetime %d h).', 'public-post-preview' ), esc_html( wp_date( 'Y-m-d H:i', self::get_link_started( $post->ID ) + self::get_link_hours( $post->ID ) * HOUR_IN_SECONDS ) ), self::get_link_hours( $post->ID ) ); ?></span>
		<?php endif; ?>
		<div style="margin-top:8px">
			<label for="gp-ppp-hours"><?php esc_html_e( 'Link lifetime (hours, 1-168):', 'public-post-preview' ); ?></label>
			<input type="number" id="gp-ppp-hours" name="gp_ppp_hours" class="small-text" min="1" max="168" step="1" value="<?php echo esc_attr( self::get_link_hours( $post->ID ) ); ?>" />
			<label style="display:block;margin-top:6px"><input type="checkbox" name="gp_ppp_regenerate" value="1" /> <?php esc_html_e( 'Regenerate link now (old shared copies expire immediately)', 'public-post-preview' ); ?></label>
			<span class="description"><?php esc_html_e( 'Save the post to apply. Regenerating stamps a fresh start time, so previously shared links stop working at once.', 'public-post-preview' ); ?></span>
		</div>
		<?php
	}
	 ...
	 *  - _ppp, a custom nonce, see GP_Public_Preview::create_nonce()
	...
	public static function register_public_preview( $post_id, $post ) {
		...
		// GP fork: capture per-post lifetime + regenerate flag (classic editor).
		$preview_post_id = (int) $post->ID;
		if ( isset( $_POST['gp_ppp_hours'] ) ) {
			$hours = (int) $_POST['gp_ppp_hours'];
			if ( $hours >= 1 && $hours <= 168 ) {
				update_post_meta( $preview_post_id, 'gp_ppp_hours', $hours );
			} else {
				delete_post_meta( $preview_post_id, 'gp_ppp_hours' );
			}
		}
		if ( ! empty( $_POST['gp_ppp_regenerate'] ) ) {
			update_post_meta( $preview_post_id, 'gp_ppp_started', time() );
		}
		...
		if ( empty( $_POST['public_post_preview'] ) && in_array( $preview_post_id, $preview_post_ids, true ) ) {
			...
			delete_post_meta( $preview_post_id, 'gp_ppp_started' ); // GP fork.
		} elseif (
			...
			update_post_meta( $preview_post_id, 'gp_ppp_started', time() ); // GP fork: fresh link window.
		} else {
	...
	private static function is_public_preview_available( $post_id ) {
		...
		if ( ! self::is_link_within_lifetime( $post_id ) ) {
			wp_die( __( 'This preview link has expired. Please ask the author to share a fresh link.', 'public-post-preview' ), 403 );
		}
	...
	/**
	 * GP fork: effective lifetime in hours for a post preview link.
	 * Per-post meta override (1-168) else the settings default.
	 *
	 * @param int $post_id Post ID.
	 * @return int Hours.
	 */
	private static function get_link_hours( $post_id ) {
		$hours = (int) get_post_meta( $post_id, 'gp_ppp_hours', true );
		if ( $hours < 1 ) {
			$hours = (int) get_option( 'public_post_preview_expiration_time' );
		}
	...
	/**
	 * GP fork: absolute timestamp of the current link generation.
	 * Falls back to the post's last modified time for legacy registrations.
	 *
	 * @param int $post_id Post ID.
	 * @return int Unix timestamp.
	 */
	private static function get_link_started( $post_id ) {
	...
	/**
	 * GP fork: is the shared link still inside its lifetime window?
	 *
	 * @param int $post_id Post ID.
	 * @return bool
	 */
	private static function is_link_within_lifetime( $post_id ) {
		return time() <= self::get_link_started( $post_id ) + self::get_link_hours( $post_id ) * HOUR_IN_SECONDS;
	}
	...
	private static function nonce_tick() {
		// Fork: the nonce window is a fixed 168h ceiling; the real per-link
		// lifetime (meta gp_ppp_hours or the settings default) is enforced by
		// the expiry gate inside is_public_preview_available().
		$nonce_life = apply_filters( 'ppp_nonce_life', 168 * HOUR_IN_SECONDS );
	...
	/**
	 * 	/**
	 * GP fork: save per-post lifetime + regenerate the link (block editor panel).
	 * Shares the upstream per-post nonce action. Editors only, drafts only.
	 */
	public static function ajax_gp_save() {
		if ( empty( $_POST['post_ID'] ) ) {
			wp_send_json_error( array( 'message' => 'incomplete_data' ) );
		}
		...
		if ( ! $post || ! current_user_can( 'edit_post', $preview_post_id ) ) {
			wp_send_json_error( array( 'message' => 'cannot_edit' ) );
		}

		if ( in_array( $post->post_status, self::get_published_statuses(), true ) ) {
			wp_send_json_error( array( 'message' => 'invalid_post_status' ) );
		}
		...
		wp_send_json_success(
			array(
				'previewUrl' => $enabled ? self::get_preview_link( $post ) : '',
				'hours'      => self::get_link_hours( $preview_post_id ),
			)
		);
	}

	/**
	 * Deletes the option 'public_post_preview'
	...
	public static function activate() {
		// GP fork: tighten the default lifetime to 24h (upstream default is 48h).
		$current = (int) get_option( 'public_post_preview_expiration_time' );
		if ( ! $current || 48 === $current ) {
			update_option( 'public_post_preview_expiration_time', 24 );
		}
		register_uninstall_hook( __FILE__, array( 'GP_Public_Preview', 'uninstall' ) );
	}
}

add_action( 'plugins_loaded', array( 'GP_Public_Preview', 'init' ) );

register_activation_hook( __FILE__, array( 'GP_Public_Preview', 'activate' ) );
